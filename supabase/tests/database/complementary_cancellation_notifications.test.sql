...
insert into complementary_cancel_tap_results select matches((select n.message from public.admin_notifications n join public.reservation_resources rr on rr.id=n.reservation_resource_id join public.reservations r on r.id=rr.reservation_id where r.customer_name='Public Complement' and r.business_id='c9100000-0000-4000-8000-000000000001' limit 1),'Churrasqueira 2.*Reserva do dia','day notification contains the resource and occupancy description');
