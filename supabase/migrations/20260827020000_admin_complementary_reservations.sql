-- Authenticated Admin surface for aggregate reservations. Public hours remain
-- enforced only by the public RPC; conflicts and allocations remain shared.

create or replace function private.attach_appointment_reservation_context()
returns trigger language plpgsql security definer set search_path = '' as $$
declare context_reservation_id uuid;
begin
  context_reservation_id := nullif(pg_catalog.current_setting('app.reservation_id', true), '')::uuid;
  if context_reservation_id is null then return new; end if;
  if not exists (
    select 1 from public.reservations reservation
    where reservation.id = context_reservation_id and reservation.business_id = new.business_id
  ) then raise exception 'reservation_context_invalid' using errcode = '23514'; end if;
  new.reservation_id := context_reservation_id;
  return new;
end;
$$;

revoke all on function private.attach_appointment_reservation_context() from public, anon, authenticated;

create or replace function public.get_admin_complementary_availability(
  p_date date, p_start_time time default null, p_end_time time default null
) returns jsonb language plpgsql stable security definer set search_path = '' as $$
declare current_user_id uuid := auth.uid(); selected_business_id uuid; selected_group record; requested_period tsrange; options jsonb;
begin
  ...
  if selected_business_id is null then raise exception 'admin_reservation_forbidden' using errcode = '42501'; end if;
  select booking_group.id, booking_group.label, booking_group.intent_name, booking_group.occupancy_mode into selected_group
  from public.booking_groups booking_group where booking_group.business_id = selected_business_id and booking_group.position = 3 and booking_group.active;
  if not found then return jsonb_build_object('configured', false, 'options', '[]'::jsonb); end if;
  if selected_group.occupancy_mode = 'day' and (p_start_time is not null or p_end_time is not null)
    then raise exception 'reservation_invalid_interval' using errcode = '22023'; end if;
  if selected_group.occupancy_mode = 'time_slot' and (p_start_time is null or p_end_time is null or p_start_time >= private.normalize_end_of_day_time(p_start_time,p_end_time))
    then raise exception 'reservation_invalid_interval' using errcode = '22023'; end if;
  requested_period := private.complementary_period(selected_group.occupancy_mode,p_date,p_start_time,p_end_time);
  select coalesce(jsonb_agg(jsonb_build_object('option_id',option.id,'name',option.name,'available',not exists(
    select 1 from public.resource_allocations allocation where allocation.business_id=selected_business_id and allocation.option_id=option.id and allocation.active and allocation.occupied_period && requested_period
  )) order by option.sort_order,option.name),'[]'::jsonb) into options
  from public.booking_options option where option.business_id=selected_business_id and option.group_id=selected_group.id and option.active;
  return jsonb_build_object('configured',true,'group_name',selected_group.label,'intent_name',selected_group.intent_name,'occupancy_mode',selected_group.occupancy_mode,'reservation_date',p_date,'start_time',p_start_time,'end_time',p_end_time,'options',options);
end;
$$;

revoke all on function public.get_admin_complementary_availability(date,time,time) from public, anon;
grant execute on function public.get_admin_complementary_availability(date,time,time) to authenticated;
...
begin
  ...
  if has_primary then
    ...
    exception when others then raise exception 'reservation_payload_invalid' using errcode='22023'; end;
