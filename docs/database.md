...
- `reservation_series`: recorrência semanal Admin Complementar-only, finita (2–260) ou permanente (`repeat_count = null`, horizonte hoje + 90 dias). `recurrence_payload` preserva o modelo validado para materialização idempotente por `materialize_recurring_reservations`; `reservations.series_id/series_date` identificam ocorrências. A criação inicial e cada expansão reutilizam criação/allocations existentes e rejeitam o lote inteiro se houver conflito, informando suas datas. `cancelled_from` encerra a série inclusive nessa data; cancelamento futuro e corte são atômicos em `cancel_admin_reservation_series`, com o mesmo lock do materializador. Cancelamento individual preserva a identidade e não recria a ocorrência. `day` não recebe horário fictício; recorrências de Principal e Complementar são independentes. Reservas `combined` são sempre avulsas e não suportam recorrência, por regra definitiva; a UI não oferece repetição e o backend rejeita essa tentativa. Recorrência pública permanece fora do escopo atual; renovação automática das permanentes é melhoria futura, e edição/exclusão de série ficam fora desta PR. Ver [arquitetura específica](architecture-complementary-weekly-recurrence.md).
