...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [configuration, notificationFeed, modules] = await Promise.all([
    getBusinessConfiguration(currentBusiness.id),
    getAdminNotificationFeed(currentBusiness.id),
    getBusinessModules(currentBusiness.id),
  ]);
  ...
  return <>
    ...
    <AdminShell modules={modules} currentBusiness={currentBusiness} platformAdmin={platformAdmin} logoUrl={configuration.logoUrl} palette={getPalette(configuration.paletteId)} initialTheme={configuration.themePreference} notificationFeed={notificationFeed} vapidPublicKey={pushServerConfigured ? process.env.VAPID_PUBLIC_KEY ?? null : null} user={{ id: user.id, name: profile?.name?.trim() || email.split("@")[0], email }}>{children}</AdminShell>
  </>;
}
