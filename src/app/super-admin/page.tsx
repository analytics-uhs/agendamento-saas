...
export default async function SuperAdminDashboardPage() {
  ...
  return <><div className="flex items-end justify-between gap-4"><PageHeading title="Visão geral" description="Acompanhe os principais números da plataforma." /><Link href="/super-admin/negocios" className="focus-ring hidden rounded-xl border bg-background px-4 py-2 text-sm font-semibold hover:bg-surface sm:block">Ver negócios</Link></div>
    <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-3">{cards.map(({ label, value, Icon }) => <article key={label} className="rounded-xl border bg-background p-4 sm:p-5"><Icon className="h-5 w-5 text-primary" /><p className="mt-4 text-2xl font-semibold tabular-nums">{value}</p><p className="mt-1 text-xs text-muted sm:text-sm">{label}</p></article>)}</div>
    <section className="mt-6 rounded-xl border bg-background p-5"><h2 className="font-semibold">Administração da plataforma</h2><p className="mt-1 text-sm text-muted">Consulte configurações e atividade dos estabelecimentos ou altere seu status operacional.</p><Link href="/super-admin/negocios" className="focus-ring mt-4 inline-flex rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white">Gerenciar negócios</Link></section>
  </>;
}
