...
import { ArrowLeft, CalendarClock, ExternalLink, Mail, Phone, UserRound } from "lucide-react";
...
export default async function PlatformBusinessDetailPage({ params }: { params: Promise<{ businessId: string }> }) {
  ...
  return <><Link href="/super-admin/negocios" className="focus-ring inline-flex items-center gap-1 rounded-lg text-sm font-medium text-muted hover:text-foreground"><ArrowLeft className="h-4 w-4" />Negócios</Link>
    ...
    <section className="mt-5 rounded-xl border bg-background p-5"><h2 className="font-semibold">Horários de funcionamento</h2><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{detail.hours.map((hour) => <div key={hour.weekday} className="flex items-center justify-between gap-3 rounded-xl border px-3 py-2 text-sm"><span>{weekdays[hour.weekday]}</span><span className={hour.active ? "font-medium" : "text-muted"}>{hour.active ? `${hour.startTime}–${hour.endTime}` : "Fechado"}</span></div>)}</div></section>
