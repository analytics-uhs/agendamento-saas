...
export async function listAdminComplementaryReservations(businessId: string, startDate: string, endDate = startDate): Promise<AdminComplementaryReservation[]> {
  ...
  const [resourcesResult, reservationsResult] = await Promise.all([
    ...
    supabase.from("reservations").select("id, customer_name, customer_whatsapp, series_id").eq("business_id", businessId),
  ]);
  ...
  return (resourcesResult.data ?? []).flatMap((resource) => {
    ...
    return [{ id: resource.id, reservationId: resource.reservation_id, seriesId: reservation.series_id, optionId: resource.option_id, customerName: reservation.customer_name, customerWhatsapp: reservation.customer_whatsapp, reservationDate: resource.reservation_date, startTime: resource.start_time?.slice(0, 5) ?? null, endTime: resource.end_time ? displayEndTime(resource.end_time) : null, occupancyMode: resource.occupancy_mode, status: resource.status, groupName: resource.group_name_snapshot, optionName: resource.option_name_snapshot }];
  });
}
...
export async function createAdminReservation(input: ManualReservationInput, businessId?: string): Promise<AppointmentRepositoryError | null> {
  if (input.repeatCount !== undefined && (!businessId || input.intent !== "complementary" || input.primary !== null)) {
    return { message: "reservation_series_invalid", code: "22023" };
  }
  const supabase = await createClient();
  ...
  const { error } = input.repeatCount !== undefined && businessId
    ? await supabase.rpc("create_admin_reservation_series", { p_business_id: businessId, p_payload: payload, p_repeat_count: input.repeatCount })
    : await supabase.rpc("create_admin_reservation", { p_payload: payload });
  return error;
}
