...
export async function getBusinessConfiguration(businessId: string): Promise<BusinessForm> {
  ...
  const [businessResult, groupsResult, optionsResult, hoursResult, settingsResult] = await Promise.all([
    ...
    supabase.from("business_settings").select("duration_mode, fixed_duration_minutes, minimum_booking_notice_minutes, public_booking_start_order, palette, theme_preference").eq("business_id", businessId).single(),
  ]);
  ...
  return {
    ...
    publicBookingStartOrder: settingsResult.data.public_booking_start_order,
