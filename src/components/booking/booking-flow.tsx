...
import { ArrowLeft, CalendarDays, Check, Clock3, Info, Layers3, LoaderCircle, MapPin, UserRound } from "lucide-react";
import { useEffect, useMemo, useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { createPublicReservation, getAvailability, getComplementaryAvailability, getComplementaryTimeSlots } from "@/app/agendar/[slug]/actions";
...
function StepHeading({ number, title, description }: { number: number; title: string; description?: string }) {
  return <div className="flex items-start gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-primary text-xs font-bold text-white">{number}</span><div><h2 tabIndex={-1} className="focus-ring rounded-sm text-base font-semibold sm:text-lg">{title}</h2>{description ? <p className="mt-0.5 text-xs leading-relaxed text-muted sm:text-sm">{description}</p> : null}</div></div>;
}
...
function CompletedStep({ label, value }: { label: string; value: string }) {
  return <Card as="section" padding="sm" className="flex items-center gap-3 bg-card"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary"><Check className="h-4 w-4" /></span><div className="min-w-0 flex-1"><p className="truncate text-xs text-muted">{label}</p><p title={value} className="truncate text-sm font-semibold">{value}</p></div></Card>;
}
function FlowProgress({ steps, activeStep }: { steps: ReturnType<typeof publicBookingSteps>; activeStep: PublicBookingStepId }) {
  ...
  const nextLabel = activeStep === "intent" && steps.length === 1 ? "Escolha como começar" : activeIndex === steps.length - 1 ? "Quase lá" : `Depois: ${steps[activeIndex + 1]?.label ?? "confirmação"}`;
  return <div className="rounded-xl border bg-card px-4 py-3" aria-label={`Etapa ${activeIndex + 1} de ${steps.length}: ${steps[activeIndex]?.label}`}><div className="flex items-center justify-between gap-3 text-xs"><p className="min-w-0 truncate font-semibold text-primary">Etapa {activeIndex + 1} de {steps.length} · {steps[activeIndex]?.label}</p><p className="max-w-[45%] shrink-0 truncate text-muted">{nextLabel}</p></div><div className="mt-2 flex gap-1.5" aria-hidden="true">{steps.map((step, index) => <span key={step.id} className={classes("h-0.5 flex-1 rounded-full bg-border", index < activeIndex && "bg-primary", index === activeIndex && "bg-accent")} />)}</div></div>;
}
function OptionButton({ selected, name, meta, disabled, onClick }: { selected: boolean; name: string; meta?: string; disabled?: boolean; onClick: () => void }) {
  return <button type="button" aria-pressed={selected} disabled={disabled} onClick={onClick} className={classes("focus-ring flex min-h-12 w-full items-center gap-3 rounded-xl border bg-card px-3 py-3 text-left transition-colors hover:border-primary disabled:cursor-not-allowed disabled:opacity-45", selected && "border-primary bg-primary/10")}><span className={classes("grid h-7 w-7 shrink-0 place-items-center rounded-lg border bg-card text-transparent", selected && "border-primary bg-primary text-white")}><Check className="h-4 w-4" /></span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{name}</span>{meta ? <span className="text-xs text-muted">{meta}</span> : null}</button>;
}
...
function previewData(state: MockAppState, paletteId?: string, themePreference: VisualThemePreference = "light"): PublicBookingData {
  ...
  const group = (position: 1 | 2, label: string, options: { id: string; name: string; durationMinutes?: number }[]) => ({ position, label, required: true, intentName: null, occupancyMode: null, options: options.map((option) => ({ id: option.id, name: option.name, durationMinutes: option.durationMinutes ?? null })) });
  return { business: { id: "preview", name: state.business.name, slug: state.business.slug, whatsapp: state.business.whatsapp, logoUrl: null, address: null, googleMapsUrl: null, instagramUrl: null, facebookUrl: null }, groups: [...(state.group1.enabled ? [group(1, state.group1.label, state.group1.options)] : []), ...(state.group2.enabled ? [group(2, state.group2.label, state.group2.options)] : [])], hours: state.hours.filter((hour) => hour.enabled).map((hour) => ({ weekday: dayToWeekday[hour.day], startTime: hour.start, endTime: hour.end })), settings: { durationMode: state.duration.mode === "fixed-multiple" ? "fixed_multiple" : state.duration.mode === "group2" ? "group_2" : "fixed", fixedDurationMinutes: state.duration.fixedMinutes, allowMultipleBlocks: state.duration.mode === "fixed-multiple", palette: getPalette(paletteId ?? state.paletteId), themePreference } };
}

function localSlots(booking: PublicBookingData, date: string, duration: number) {
  return civilDayWindows(booking.hours, parseISO(date).getDay()).flatMap(({ start, end }) => {
    const slots: BookingSlot[] = [];
    for (let minute = start; minute + duration <= end && minute < 1440; minute += duration) {
      if (minute >= 0) slots.push({ startTime: minutesToTime(minute), durationMinutes: duration, maxBlocks: 1 });
    }
    return slots;
...
export function BookingFlow({ booking: bookingProp, preview = false, paletteId, themePreference }: { booking?: PublicBookingData; preview?: boolean; paletteId?: string; themePreference?: VisualThemePreference }) {
  ...
  const [complementaryOptions, setComplementaryOptions] = useState<ComplementaryAvailabilityOption[]>([]); const [customer, setCustomer] = useState(""); const [whatsapp, setWhatsapp] = useState(""); const [message, setMessage] = useState<string | null>(null); const [sequenceMessage, setSequenceMessage] = useState<string | null>(null);
  const requestRef = useRef(0); const [isLoadingSlots, startSlotsTransition] = useTransition(); const [isLoadingComplementary, startComplementaryTransition] = useTransition(); const [isSubmitting, startSubmitTransition] = useTransition();
  ...
  const complementaryDone = !includesComplementary || Boolean(complementary); const timeRequired = includesPrimary || complementaryGroup?.occupancyMode === "time_slot";
  const canConfirm = Boolean(intent && date && (!timeRequired || time) && group1Done && group2Done && complementaryDone && customer.trim().length >= 2 && whatsapp.trim() && !isSubmitting);
  const configurationInvalid = Boolean((groupOne && groupOne.options.length === 0) || (groupTwo && groupTwo.options.length === 0) || (complementaryGroup && complementaryGroup.options.length === 0));
  const hasProgress = Boolean((complementaryGroup && intent) || group1 || group2 || date || time || complementary); const activeStepIndex = steps.findIndex((step) => step.id === activeStep); const style = appearanceStyle(booking.settings.palette, booking.settings.themePreference);
  const ctaHelper = bookingCtaHelper({ groupOneMissing: !group1Done, groupTwoMissing: !group2Done, dateMissing: !date, timeMissing: timeRequired && !time, complementaryMissing: !complementaryDone, customerMissing: customer.trim().length < 2, whatsappMissing: !whatsapp.trim() });
  const numberOf = (id: PublicBookingStepId) => steps.findIndex((step) => step.id === id) + 1;
  const intentLabel = intent && complementaryGroup ? intentOptions(groupOne, complementaryGroup).find((option) => option.id === intent)?.name : null;

  const previousStep = previousPublicBookingStep(activeStep, steps);
  const stepContentRef = useRef<HTMLElement>(null);
  const lastFocusedStep = useRef(activeStep);
  useEffect(() => {
    if (lastFocusedStep.current !== activeStep) {
      lastFocusedStep.current = activeStep;
      stepContentRef.current?.querySelector<HTMLHeadingElement>("h2")?.focus();
    }
  }, [activeStep]);
  const navigationPending = isLoadingSlots || isLoadingComplementary || isSubmitting;
  function goBack() {
    if (previousStep && !navigationPending) { setMessage(null); setActiveStep(previousStep); }
  }
  ...
  function fetchComplementary(selectedDate: string, selectedStart: string | null, selectedEnd: string | null) {
    ...
    if (preview) { setComplementaryOptions(complementaryGroup?.options.map((option) => ({ id: option.id, name: option.name, available: true })) ?? []); return; }
  ...
  function loadDate(selectedDate: string) {
    ...
    setActiveStep("time");
    if (intent === "complementary" && !preview) {
  ...
  function finishTimeSelection(selectedStart: string, selectedEnd: string, keepSelection = false) {
    if (includesComplementary) {
      if (keepSelection && complementary && complementaryOptions.length) nextAfter("time");
      else fetchComplementary(date!, selectedStart, selectedEnd);
    } else setActiveStep("customer");
  }
  function chooseTime(slot: BookingSlot) {
    setMessage(null);
    if (booking.settings.durationMode === "fixed_multiple" && includesPrimary) { const next = selectFixedMultipleSlot(slots, time, blocks, slot.startTime); if (next.startTime !== time || next.blocks !== blocks) { setComplementary(null); setComplementaryOptions([]); } setTime(next.startTime); setBlocks(next.blocks); setSequenceMessage(next.rejected ? "Este horário não pode ser combinado. Selecione horários consecutivos." : null); return; }
    setTime(slot.startTime); setBlocks(1); finishTimeSelection(slot.startTime, fixedMultipleEndTime(slot.startTime, slot.durationMinutes, 1), slot.startTime === time);
  }
  ...
  function completedStepValue(id: PublicBookingStepId) {
    if (id === "intent") return intentLabel; if (id === "group_1") return selectedGroupOne?.name ?? null; if (id === "group_2") return selectedGroupTwo?.name ?? null; if (id === "date") return date ? formatLongDate(date) : null;
    if (id === "customer") return customer.trim() || null;
    if (id === "time") return time ? `${time}${endTime ? `–${endTime}` : ""} · ${formatDuration(duration)}` : null; if (id === "complementary") return selectedComplementary?.name ?? null; return null;
  }

  function confirm() {
    if (preview || !intent || !date || !canConfirm || activeStep !== "review") return; setMessage(null);
    const payload = buildPublicReservationPayload({ intent, group1OptionId: group1, group2OptionId: group2, complementaryOptionId: complementary, occupancyMode: complementaryGroup?.occupancyMode ?? null, date, startTime: time, endTime, blocks, customerName: customer, customerWhatsapp: whatsapp });
    startSubmitTransition(async () => { const result = await createPublicReservation({ slug: booking.business.slug, payload }); if (!result.ok) { if (result.staleSelection) { setComplementary(null); setComplementaryOptions([]); } if (result.conflict) { if (result.message.includes("recurso") && date) fetchComplementary(date, time, endTime); else if (date) loadDate(date); } setMessage(result.message); return; }
      sessionStorage.setItem(`booking-confirmation:${booking.business.slug}`, JSON.stringify({ ...result.data, business: { ...result.data.business, whatsapp: booking.business.whatsapp }, appearance: { palette: booking.settings.palette, themePreference: booking.settings.themePreference } } satisfies BookingConfirmation)); router.push(`/agendar/${booking.business.slug}/confirmacao`); });
  }

  const summary = <div className="space-y-4"><div className="flex items-center gap-3"><BusinessLogo name={booking.business.name} logoUrl={booking.business.logoUrl} size="sm" /><div><p className="text-sm font-semibold">Sua reserva</p><p className="text-xs text-muted">{booking.business.name}</p></div></div><dl className="space-y-3 text-sm">{selectedGroupOne ? <div className="flex justify-between gap-4"><dt className="text-muted">{groupOne?.label}</dt><dd className="text-right font-semibold">{selectedGroupOne.name}</dd></div> : null}{selectedGroupTwo ? <div className="flex justify-between gap-4"><dt className="text-muted">{groupTwo?.label}</dt><dd className="text-right font-semibold">{selectedGroupTwo.name}</dd></div> : null}{selectedComplementary ? <div className="flex justify-between gap-4"><dt className="text-muted">{complementaryGroup?.label}</dt><dd className="text-right font-semibold">{selectedComplementary.name}</dd></div> : null}{date ? <div className="flex justify-between gap-4"><dt className="flex items-center gap-1.5 text-muted"><CalendarDays className="h-3.5 w-3.5" />Data</dt><dd className="text-right font-semibold capitalize">{formatLongDate(date)}</dd></div> : null}{time ? <div className="flex justify-between gap-4"><dt className="flex items-center gap-1.5 text-muted"><Clock3 className="h-3.5 w-3.5" />Horário</dt><dd className="text-right font-semibold">{time}{endTime ? `–${endTime}` : ""}</dd></div> : selectedComplementary && complementaryGroup?.occupancyMode === "day" ? <div className="flex justify-between gap-4"><dt className="text-muted">Período</dt><dd className="font-semibold">Reserva do dia</dd></div> : null}{customer.trim() ? <div className="flex justify-between gap-4"><dt className="flex items-center gap-1.5 text-muted"><UserRound className="h-3.5 w-3.5" />Nome</dt><dd className="text-right font-semibold">{customer.trim()}</dd></div> : null}</dl>{time ? <div className="flex items-center justify-between border-t pt-3 text-sm"><span className="text-muted">Duração total</span><strong>{formatDuration(duration)}</strong></div> : null}</div>;
  const confirmationAction = <><Button className="h-12 w-full text-base" disabled={!canConfirm || preview} onClick={confirm}>{isSubmitting ? <><LoaderCircle className="h-4 w-4 animate-spin" />Confirmando...</> : "Confirmar reserva"}</Button><p className="mt-2 flex items-start justify-center gap-1.5 text-center text-xs leading-relaxed text-muted"><Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />{ctaHelper}</p></>;
  ...
  return <div style={style} data-theme={booking.settings.themePreference} className="min-h-screen bg-surface text-foreground"><div className={classes("mx-auto w-full max-w-6xl px-4 pb-16 pt-5 sm:px-6 sm:pt-7", preview && "scale-[0.98]")}>
    ...
    {!configurationInvalid ? <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]"><main ref={stepContentRef} className="space-y-3">
      ...
      {activeStep === "review" ? <ExpandedStep number={numberOf("review")} title="Revise sua reserva" description="Confira suas escolhas antes de confirmar."><div className="lg:hidden">{summary}</div><p className="hidden text-sm text-muted lg:block">Se precisar revisar alguma escolha, use Voltar.</p></ExpandedStep> : null}
      <nav aria-label="Navegação do agendamento" className="flex flex-wrap items-start gap-3 pt-2">
        {previousStep ? <Button variant="ghost" disabled={navigationPending} onClick={goBack}><ArrowLeft className="h-4 w-4" aria-hidden="true" />Voltar</Button> : null}
        {activeStep === "review" ? <div className="min-w-0 flex-1">{confirmationAction}</div> : canContinue || activeStep === "customer" ? <Button className="ml-auto" disabled={!canContinue || navigationPending} onClick={continueStep}>Continuar</Button> : null}
      </nav>
    </main><aside className="sticky top-6 hidden lg:block"><Card padding="md" className="bg-card">{summary}</Card></aside></div> : null}
