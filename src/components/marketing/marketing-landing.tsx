...
import agendaFacilLogo from "@/app/icon.png";
import { FOUNDER_OFFER, MARKETING_TRIAL_HREF, founderOfferAvailability } from "@/lib/marketing";
...
function Brand({ priority = false }: { priority?: boolean }) {
  return <span className="sm-brand"><Image className="sm-brand-logo" src={agendaFacilLogo} alt="AgendaFácil" sizes="42px" priority={priority} /> <span aria-hidden="true">AgendaFácil</span></span>;
}
...
export function MarketingLanding() {
  ...
  const availability = founderOfferAvailability();
  const progressStyle = { "--founder-progress": `${availability.occupiedPercentage}%` } as CSSProperties;
  ...
  return <div id="agenda-motion" ref={rootRef}>
    ...
    <main id="conteudo">
      ...
      <section className="sm-section sm-founders" id="preco"><div className="sm-shell"><div className="sm-offer sm-reveal"><div><span className="sm-offer-tag">LOTE FUNDADORES · PRIMEIROS 50 NEGÓCIOS</span><h2>Comece sem compromisso. Continue como fundador.</h2><p>Experimente por 15 dias sem cartão. Se decidir continuar, o valor de fundador permanece enquanto sua assinatura estiver ativa.</p><div className="sm-offer-benefits"><div className="sm-offer-benefit"><i>✓</i>15 dias grátis</div><div className="sm-offer-benefit"><i>✓</i>Sem cartão</div><div className="sm-offer-benefit"><i>✓</i>Cancele quando quiser</div><div className="sm-offer-benefit"><i>✓</i>Condição mantida enquanto ativo</div></div></div><div className="sm-price-panel"><div className="sm-official">Preço oficial: <s>R$ {FOUNDER_OFFER.officialPrice}/mês</s></div><div className="sm-price"><sup>R$</sup>{FOUNDER_OFFER.currentPrice} <small>/mês</small></div><div className="sm-price-note">para os primeiros {availability.totalSpots} negócios</div><div className="sm-seats-copy"><b>{availability.occupiedSpots} de {availability.totalSpots} ocupadas</b><span>{availability.availableSpots} vagas disponíveis</span></div><div className="sm-seat-meter" style={progressStyle}><div className="sm-seat-progress" role="progressbar" aria-label={`${availability.occupiedSpots} de ${availability.totalSpots} vagas ocupadas`} aria-valuenow={availability.occupiedSpots} aria-valuemin={0} aria-valuemax={availability.totalSpots}><span /></div><div className="sm-seat-percent"><strong>{availability.occupiedPercentage}% ocupadas</strong><span>Oferta para os primeiros {availability.totalSpots} negócios</span></div></div><TrialLink /><div className="sm-risk">15 dias grátis · Sem cartão</div><div className="sm-next-price">Próximo lote: R$ {FOUNDER_OFFER.nextPrice}/mês. Depois, preço oficial de R$ {FOUNDER_OFFER.officialPrice}/mês.</div></div></div></div></section>
