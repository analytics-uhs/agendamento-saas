...
export function BusinessPageContent({ initialBusiness }: { initialBusiness: BusinessForm }) {
  ...
  return <><PageHeading title="Meu negócio" description="Informações exibidas na página pública." /><div className="mt-6 space-y-6">
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-background p-4"><div className="min-w-0"><p className="text-xs text-muted">Sua página</p><a className="focus-ring inline-flex max-w-full items-center gap-1 truncate rounded text-sm font-medium hover:text-primary" href={link} target="_blank" rel="noopener noreferrer">{publicDomain}/{business.slug}<ExternalLink className="h-3.5 w-3.5 shrink-0" /></a></div><Button variant="outline" size="sm" onClick={async () => { await navigator.clipboard?.writeText(link); setCopied(true); window.setTimeout(() => setCopied(false), 1600); }}>{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? "Copiado" : "Copiar link"}</Button></section>
  </div></>;
}
