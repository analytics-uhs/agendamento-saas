...
export function BusinessPageContent({ initialBusiness, platformBusinessId }: { initialBusiness: BusinessForm; platformBusinessId?: string }) {
  ...
  return <><PageHeader title="Meu negócio" description="Informações exibidas na página pública." /><div className="mt-6 space-y-6">
    <Card as="section" padding="md" className="space-y-5">
      <LogoUploader platformBusinessId={platformBusinessId} businessId={business.id!} businessName={business.name} logoUrl={business.logoUrl} onUploaded={(url, uploadResult) => { if (uploadResult.ok) setBusiness((current) => ({ ...current, logoUrl: url })); setResult(uploadResult); }} />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3"><SaveNotice result={result} /><Button disabled={pending} onClick={() => startTransition(async () => setResult(await saveBusiness(business, platformBusinessId)))}>{pending ? "Salvando..." : "Salvar alterações"}</Button></div>
    </Card>
