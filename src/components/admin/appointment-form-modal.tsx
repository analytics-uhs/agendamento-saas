...
export function AppointmentFormModal({
  ...
  function submit() {
    ...
    startSaving(async () => {
      ...
      const result = !editing && complementaryGroup && (!recurring || intent === "complementary")
        ? await createManualReservation({ repeatCount: recurring && intent === "complementary" ? repeatCount : undefined, intent, primary: includesPrimary ? input : null, complementary: includesComplementary && complementaryOptionId ? { optionId: complementaryOptionId, occupancyMode: complementaryGroup.occupancyMode, date, startTime: complementaryGroup.occupancyMode === "time_slot" ? (intent === "combined" ? selectedStartTime : complementaryStartTime) : null, endTime: complementaryGroup.occupancyMode === "time_slot" ? (intent === "combined" ? primaryEndTime : complementaryEndTime) : null } : null, customerName, customerWhatsapp })
        : editing && appointment
  ...
  return (
    <Modal title={editing ? "Editar agendamento" : "Novo agendamento"} onClose={onClose}>
      <div className="p-4 sm:p-5">
        ...
        {includesPrimary && outsideBusinessHours ? <p role="status" className="mt-4 flex items-start gap-2 rounded-xl border border-accent/40 bg-accent/10 p-3 text-sm text-foreground"><Info className="mt-0.5 h-4 w-4 shrink-0 text-accent" /><span>Este horário está fora do funcionamento configurado. O agendamento será criado somente pelo Admin e não abrirá disponibilidade na página pública.</span></p> : null}
        {!editing && (intent === "primary" || (intent === "complementary" && complementaryOptionId)) ? <div className="mt-4 rounded-xl border bg-surface/50 p-4"><label className="flex cursor-pointer items-center gap-3 text-sm font-medium"><input type="checkbox" className="h-4 w-4 accent-primary" checked={recurring} onChange={(event) => setRecurring(event.target.checked)} />Repetir semanalmente</label>{recurring ? <div className="mt-4 space-y-3 border-t pt-4">{!includesComplementary && <div className="grid gap-2 sm:grid-cols-2"><label className="flex items-center gap-2 rounded-xl border bg-background p-3 text-sm"><input type="radio" name="modal-recurrence" checked={recurrenceType === "permanent"} onChange={() => setRecurrenceType("permanent")} />Permanente</label><label className="flex items-center gap-2 rounded-xl border bg-background p-3 text-sm"><input type="radio" name="modal-recurrence" checked={recurrenceType === "count"} onChange={() => setRecurrenceType("count")} />Quantidade de repetições</label></div>}{includesComplementary || recurrenceType === "count" ? <div className="space-y-2"><Label htmlFor="weekly-occurrence-count">Quantidade de ocorrências semanais</Label><Input id="weekly-occurrence-count" type="number" min={2} max={260} value={repeatCount} onChange={(event) => setRepeatCount(Number(event.target.value))} /></div> : null}{includesPrimary && startTime ? <p className="flex items-center gap-2 text-sm font-medium text-primary"><Repeat2 className="h-4 w-4" />{recurrenceSummary(date, startTime, includesComplementary || recurrenceType === "count" ? repeatCount : null)}</p> : null}</div> : null}</div> : null}
        {includesPrimary && startTime ? <p className="mt-4 text-sm text-muted">{formatLongDate(date)} · {startTime} · {duration ? formatDuration(duration) : "duração inválida"}</p> : null}
