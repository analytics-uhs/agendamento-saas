...
export function AppointmentFormModal({
  ...
  }, [appointment, date, group1OptionId, group2OptionId, intent]);
  ...
  function submit() {
    if (includesPrimary && !startTime) return;
  ...
  return (
    <Modal title={editing ? "Editar agendamento" : "Novo agendamento"} onClose={onClose}>
      <div className="p-4 sm:p-5">
        ...
        {includesPrimary ? <div className="mt-4"><p className="mb-2 text-sm font-medium">Horário</p>{loading ? <EmptyState className="flex items-center justify-center gap-2"><LoaderCircle className="h-4 w-4 animate-spin" />Consultando disponibilidade...</EmptyState> : slots.length ? <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">{slots.map((slot) => <button key={slot.startTime} type="button" onClick={() => { if (config.durationMode === "fixed_multiple") { const next = selectFixedMultipleSlot(slots, startTime, blocks, slot.startTime); setStartTime(next.startTime); setBlocks(next.blocks); setSequenceMessage(next.rejected ? "Os horários selecionados precisam ser seguidos." : null); } else { setStartTime(slot.startTime); setBlocks(1); } }} className={classes("focus-ring rounded-xl border bg-card py-2.5 text-sm font-semibold", selectedTimes.includes(slot.startTime) && "border-primary bg-primary text-white")}>{slot.startTime}</button>)}</div> : <EmptyState>Nenhum horário disponível nesta data.</EmptyState>}</div> : complementaryGroup?.occupancyMode === "day" ? <p className="mt-4 rounded-xl border bg-surface/50 p-3 text-sm text-muted">Reserva válida durante todo o dia, sem horário artificial.</p> : null}
