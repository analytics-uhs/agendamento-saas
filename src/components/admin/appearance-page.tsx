...
export function AppearancePageContent({ initialBusiness, platformBusinessId }: { initialBusiness: BusinessForm; platformBusinessId?: string }) {
  ...
  return <><PageHeader title="Aparência" description="Personalize as cores e visualize a página pública." />
    <Card as="section" padding="md" className="mt-6"><LogoUploader platformBusinessId={platformBusinessId} businessId={business.id!} businessName={business.name} logoUrl={business.logoUrl} onUploaded={(url, uploadResult) => { if (uploadResult.ok) setBusiness((current) => ({ ...current, logoUrl: url })); setResult(uploadResult); }} /></Card>
    ...
    <Card as="section" padding="sm" className="mt-6 flex items-center justify-between"><div><h2 className="text-sm font-semibold">Tema da página pública</h2><p className="text-xs text-muted">Alterne pelo ícone.</p></div><ThemePreferenceToggle value={business.themePreference} onChange={setThemePreference} /></Card>
    <div className="mt-6 flex flex-wrap items-center justify-between gap-3"><SaveNotice result={result} /><Button disabled={pending} onClick={() => startTransition(async () => setResult(await saveAppearance({ paletteId: business.paletteId, themePreference: business.themePreference }, platformBusinessId)))}>{pending ? "Salvando..." : "Salvar aparência"}</Button></div>
