...
export function AdminShell({ children, currentBusiness, platformAdmin, logoUrl, palette, initialTheme, user }: { children: React.ReactNode; currentBusiness: CurrentBusiness; platformAdmin: boolean; logoUrl: string | null; palette: BusinessPalette; initialTheme: VisualThemePreference; user: { name: string; email: string } }) {
  ...
  return <BusinessAppearance palette={palette} initialTheme={initialTheme}><div className="min-h-screen bg-surface">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r bg-background px-4 py-6 lg:flex">
      <Link href="/admin" className="focus-ring flex min-w-0 items-center gap-2 rounded-xl px-2"><BusinessLogo name={business.name} logoUrl={logoUrl} /><span className="truncate font-semibold">{business.name}</span></Link>
    ...
    <header className="sticky top-0 z-20 flex items-center justify-between border-b bg-background px-4 py-3 lg:hidden">
      <Link href="/admin" className="flex min-w-0 items-center gap-2"><BusinessLogo name={business.name} logoUrl={logoUrl} size="sm" /><span className="truncate text-sm font-semibold">{business.name}</span></Link>
