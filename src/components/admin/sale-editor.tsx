...
export function SaleEditor({ sale, items }: { sale: Sale; items: SaleItem[] }) {
  return <><PageHeader title="Detalhe da venda" description="Esta venda já foi baixada do estoque." action={<Badge variant="success">Finalizada</Badge>} />
    <Card padding="md" className="mt-6 space-y-5"><dl className="grid gap-4 sm:grid-cols-3">
      <div><dt className="text-xs text-muted">Identificação</dt><dd className="font-medium">{sale.tab_name ?? sale.customer_name ?? "Balcão"}</dd></div>
      <div><dt className="text-xs text-muted">Método no fechamento</dt><dd>{sale.payment_method ? PAYMENT_METHODS[sale.payment_method] : "—"}</dd></div>
      ...
      <OriginPayments target={{ type: "sale", id: sale.id }} />
    </Card></>;
}
