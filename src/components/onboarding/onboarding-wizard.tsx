...
import { Check, ChevronLeft, ChevronRight, Copy, ImageUp } from "lucide-react";
...
import { cloneBusinessHourWindows, createEmptyBusinessForm, normalizeSlug, validateBusinessForm, validateBusinessGroups, validateBusinessHours, validateDuration, validateSlug } from "@/lib/business-form";
...
export function OnboardingWizard() {
  ...
  const setGroup = (index: number, group: BusinessForm["groups"][number]) => setForm((current) => ({ ...current, groups: current.groups.map((item, groupIndex) => groupIndex === index ? group : item) as typeof current.groups }));
  ...
  const next = () => {
    ...
    if (step === 1) {
      ...
      if (groupsError) return setMessage(groupsError);
