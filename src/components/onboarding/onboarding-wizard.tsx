...
import { ArrowDown, ArrowUp, Check, ChevronLeft, ChevronRight, Copy, ImageUp, Laptop, Moon, Plus, Sun, Trash2 } from "lucide-react";
...
import { createEmptyBusinessForm, normalizeSlug, validateBusinessForm, validateDuration, validateSlug } from "@/lib/business-form";
...
const themeOptions = [{ id: "light" as const, label: "Claro", Icon: Sun }, { id: "dark" as const, label: "Escuro", Icon: Moon }, { id: "system" as const, label: "Sistema", Icon: Laptop }];
...
export function OnboardingWizard() {
  ...
  const next = () => {
    ...
    if (step === 1) {
      ...
      if (groupError) return setMessage(`Revise o Grupo ${groupError.position}.`);
    ...
    if (step === 2 && form.hours.some((hour) => hour.active && hour.startTime >= hour.endTime)) return setMessage("O horário final deve ser posterior ao inicial.");
  ...
  return <main className="min-h-screen bg-surface px-4 py-8"><div className="mx-auto w-full max-w-xl">
    ...
    <div className="rounded-2xl border bg-background p-5 sm:p-6">
      {step === 0 ? <div className="space-y-5"><div className="flex items-center gap-3"><Logo name={form.name || "Negócio"} size="lg" /><div><p className="text-sm font-medium">Logo do negócio</p><p className="text-xs text-muted">PNG, JPEG ou WebP, até 2 MB.</p><input ref={fileRef} className="hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { const file = event.target.files?.[0] ?? null; const error = file ? validateLogoFile(file) : null; setMessage(error); setLogoFile(error ? null : file); }} /><Button variant="outline" size="sm" className="mt-2" onClick={() => fileRef.current?.click()}><ImageUp className="h-4 w-4" />{logoFile ? logoFile.name : "Escolher imagem"}</Button></div></div><div className="space-y-2"><Label htmlFor="business-name">Nome do negócio</Label><Input id="business-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value, slug: form.slug ? form.slug : normalizeSlug(event.target.value) })} /></div><div className="space-y-2"><Label htmlFor="business-whatsapp">WhatsApp</Label><Input id="business-whatsapp" value={form.whatsapp} onChange={(event) => setForm({ ...form, whatsapp: event.target.value })} /></div><div className="space-y-2"><Label htmlFor="business-slug">URL personalizada</Label><div className="flex items-center rounded-xl border bg-card px-3"><span className="text-sm text-muted">{publicDomain}/</span><input id="business-slug" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" value={form.slug} onChange={(event) => setForm({ ...form, slug: normalizeSlug(event.target.value) })} /></div></div></div> : null}
      {step === 1 ? <div className="space-y-5">{form.groups.map((group, index) => { const groupIndex = index as 0 | 1; const showDuration = group.position === 2 && form.durationMode === "group_2"; return <section key={group.position} className="rounded-xl border p-4"><div className="grid grid-cols-[1fr_auto] items-center gap-3"><div className="space-y-1"><Label htmlFor={`group-${group.position}-label`}>Nome do Grupo {group.position}</Label><Input id={`group-${group.position}-label`} value={group.label} onChange={(event) => setGroup(groupIndex, { label: event.target.value })} /></div><Switch checked={group.active} onChange={(active) => setGroup(groupIndex, { active })} label={`Ativar Grupo ${group.position}`} /></div><div className={classes("mt-3 space-y-2", !group.active && "opacity-45")}>{group.options.map((option, optionIndex) => <div key={optionIndex} className="flex gap-2"><Input value={option.name} onChange={(event) => setGroup(groupIndex, { options: group.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, name: event.target.value } : item) })} />{showDuration ? <Input aria-label="Duração em minutos" type="number" min={5} max={1440} step={5} className="w-24" value={option.durationMinutes ?? 30} onChange={(event) => setGroup(groupIndex, { options: group.options.map((item, itemIndex) => itemIndex === optionIndex ? { ...item, durationMinutes: Number(event.target.value) } : item) })} /> : null}<Button variant="ghost" size="icon" aria-label="Mover opção para cima" disabled={optionIndex === 0} onClick={() => moveOption(groupIndex, optionIndex, -1)}><ArrowUp className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label="Mover opção para baixo" disabled={optionIndex === group.options.length - 1} onClick={() => moveOption(groupIndex, optionIndex, 1)}><ArrowDown className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label="Remover opção" onClick={() => setGroup(groupIndex, { options: group.options.filter((_, itemIndex) => itemIndex !== optionIndex) })}><Trash2 className="h-4 w-4" /></Button></div>)}<Button variant="outline" size="sm" onClick={() => setGroup(groupIndex, { options: [...group.options, { name: "Nova opção", durationMinutes: 30 }] })}><Plus className="h-4 w-4" />Adicionar opção</Button></div></section>; })}<div className="rounded-xl border p-4"><p className="text-sm font-semibold">Modo de duração</p><div className="mt-3 grid gap-2">{[{ id: "fixed" as const, label: "Duração fixa" }, { id: "fixed_multiple" as const, label: "Duração fixa + múltiplos blocos" }, { id: "group_2" as const, label: `Duração pelo ${form.groups[1].label}` }].map((mode) => <button key={mode.id} type="button" onClick={() => setForm({ ...form, durationMode: mode.id })} className={classes("focus-ring rounded-xl border p-3 text-left text-sm font-medium", form.durationMode === mode.id && "border-primary bg-primary/5 text-primary")}>{mode.label}</button>)}</div>{form.durationMode !== "group_2" ? <div className="mt-4 space-y-2"><Label htmlFor="fixed-duration">Duração do bloco</Label><Input id="fixed-duration" type="number" min={5} max={1440} step={5} value={form.fixedDurationMinutes} onChange={(event) => setForm({ ...form, fixedDurationMinutes: Number(event.target.value) })} /></div> : null}</div></div> : null}
      {step === 2 ? <div className="space-y-2"><div className="flex justify-end"><Button variant="outline" size="sm" disabled={!monday} onClick={() => monday && setForm({ ...form, hours: form.hours.map((hour) => hour.weekday === 1 ? hour : { ...hour, startTime: monday.startTime, endTime: monday.endTime }) })}><Copy className="h-4 w-4" />Copiar segunda</Button></div>{form.hours.map((hour) => <div key={hour.weekday} className="grid gap-3 rounded-xl border p-3 sm:grid-cols-[1fr_auto] sm:items-center"><div className="flex items-center gap-3"><Switch checked={hour.active} onChange={(active) => setForm({ ...form, hours: form.hours.map((item) => item.weekday === hour.weekday ? { ...item, active } : item) })} label={`Ativar ${hour.label}`} /><span className="text-sm font-medium">{hour.label}</span></div><div className="flex gap-2"><Input type="time" className="min-w-0" disabled={!hour.active} value={hour.startTime} onChange={(event) => setForm({ ...form, hours: form.hours.map((item) => item.weekday === hour.weekday ? { ...item, startTime: event.target.value } : item) })} /><Input type="time" className="min-w-0" disabled={!hour.active} value={hour.endTime} onChange={(event) => setForm({ ...form, hours: form.hours.map((item) => item.weekday === hour.weekday ? { ...item, endTime: event.target.value } : item) })} /></div></div>)}</div> : null}
      {step === 3 ? <div className="space-y-5"><div className="grid grid-cols-2 gap-3">{palettes.map((palette) => <button key={palette.id} type="button" onClick={() => setForm({ ...form, paletteId: palette.id })} className={classes("focus-ring rounded-xl border p-3 text-left", form.paletteId === palette.id && "border-primary bg-primary/5")}><div className="flex gap-1.5">{[palette.primary, palette.accent, palette.surface, palette.text].map((color) => <span key={color} className="h-6 w-6 rounded-full border" style={{ background: color }} />)}</div><p className="mt-2 text-sm font-medium">{palette.name}</p></button>)}</div><div><Label>Preferência de tema</Label><div className="mt-2 grid grid-cols-3 gap-2">{themeOptions.map(({ id, label, Icon }) => <button key={id} type="button" onClick={() => setForm({ ...form, themePreference: id })} className={classes("focus-ring flex items-center justify-center gap-2 rounded-xl border p-3 text-sm", form.themePreference === id && "border-primary bg-primary/5 text-primary")}><Icon className="h-4 w-4" />{label}</button>)}</div></div></div> : null}
